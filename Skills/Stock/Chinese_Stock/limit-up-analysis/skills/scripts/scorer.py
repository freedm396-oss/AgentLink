    ...
    def calc_capital_flow(self, code: str, detail: Dict) -> float:
        ...
        try:
            ...
            if 'realtime' in detail and not detail['realtime'].empty:
                ...
                circ_mv = rt.get('流通市值', 0)  # 元
                turnover = rt.get('换手率', 0)   # %

                if circ_mv > 0:
                    inflow_ratio = abs(main_inflow) / circ_mv * 100  # 净流入占流通市值%
                    if inflow_ratio >= 10:
                        score += 25  # 优秀（主力控盘明显）
                    elif inflow_ratio >= 5:
                        score += 18.75  # 良好
        ...
        try:
            if not all_zt_data.empty:
                ...
                if '连板数' in all_zt_data.columns:
                    ...
                    # 连板高度风险折扣（越高风险越大，不再加分）
                    if max_limit >= 7:
                        score -= 20  # ⚠️ 极高风险，7板后主力砸盘概率大
                    elif max_limit >= 5:
                        score -= 10  # ⚠️ 高风险，5板后累计涨幅大，抛压重
                    elif max_limit >= 3:
                        score -= 3   # 3板开始累计风险
